...
export async function injectTaskIntoClaudeMd(
  ...
  const section = [
    "",
    TASK_MARKER,
    "## Sandbox rules (read before touching git)",
    "",
    "`git commit` and `git push` are intercepted in this sandbox and will",
    "fail — the bind-mounted git index is incompatible across host/sandbox",
    "git versions, and the sandbox has no remote auth. Always use the",
    "`shipyard:sandbox` CLI for commits and pushes:",
    "",
    ...
    "You CAN run `yarn install` and start the dev server; the host forwards your sandbox port to the browser automatically.",
    "",
    "## Current Task",
    "",
