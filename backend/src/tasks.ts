...
export async function injectTaskIntoClaudeMd(
  ...
  const section = [
    ...
    "## Sandbox rules",
    "",
    "- Stage all changes and commit with `shipyard:sandbox commit -m \"msg\"`.",
    "  Use `--amend` (with or without `-m`) to fix the previous commit.",
    "- Push and create/update the PR with `shipyard:sandbox push`.",
    "- Do NOT run `git commit` or `git push` directly — both must go through the host.",
